...
public class IOUtils {
	...
	public static final String DOWNLOAD_PATH = new File(
			Environment.getExternalStorageDirectory(), "paranoidota/")
			.getAbsolutePath();
	...
	private static void readMounts(Context context) {
		...
		boolean addExternal = mounts.size() == 1
		...
		for (int i = 0; i < mounts.size(); i++) {
			...
			if (!vold.contains(mount)
					|| (!root.exists() || !root.isDirectory() || !root
							.canWrite())) {
				mounts.remove(i--);
			}
		}
	...
	public static Properties getDictionary(Context context) {
		if (sDictionary == null) {
			...
			try {
				sDictionary.load(context.getAssets().open(
