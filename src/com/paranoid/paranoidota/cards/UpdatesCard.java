...
public class UpdatesCard extends Card implements UpdaterListener,
	...
	public UpdatesCard(Context context, AttributeSet attrs,
		...
		if (savedInstanceState != null) {
			List<PackageInfo> mRoms = (List) savedInstanceState
					.getSerializable(ROMS);
			List<PackageInfo> mGapps = (List) savedInstanceState
					.getSerializable(GAPPS);

			mRomUpdater.setLastUpdates(mRoms.toArray(new PackageInfo[mRoms
					.size()]));
			mGappsUpdater.setLastUpdates(mGapps.toArray(new PackageInfo[mGapps
					.size()]));
		}
		...
		mDownload.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onClick(int id) {
				MainActivity activity = (MainActivity) getContext();
				activity.setState(MainActivity.STATE_DOWNLOAD, true,
						getPackages(), null, null, false, false);
			}
	...
	private void unckeckCheckBoxes(PackageInfo master) {
		...
		for (int i = 0; i < mLayout.getChildCount(); i++) {
			...
			if (view instanceof CheckBox) {
				...
				if (masterIsGapps == isGapps
						&& !masterFileName.equals(fileName)) {
	...
	@SuppressLint("ResourceAsColor") private void addPackages(PackageInfo[] packages) {
		Context context = getContext();
		Resources res = context.getResources();
		for (int i = 0; packages != null && i < packages.length; i++) {
			...
			mAdditional.addView(text);
			text = new TextView(context);
			text.setText(res.getString(R.string.update_host,
					packages[i].getHost()));
			text.setTextSize(TypedValue.COMPLEX_UNIT_PX,
					res.getDimension(R.dimen.card_small_text_size));
			text.setTextColor(R.color.card_text);
