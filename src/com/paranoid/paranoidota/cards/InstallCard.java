...
public class InstallCard extends Card implements RequestFileCallback {
	...
	public InstallCard(Context context, AttributeSet attrs,
			RebootHelper rebootHelper, Bundle savedInstanceState) {
		...
		mInstall.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onClick(int id) {
				...
				mRebootHelper.showRebootDialog(getContext(), items,
						mBackup.isChecked(), mWipeData.isChecked(),
						mWipeCaches.isChecked());
			}
		...
		if (savedInstanceState != null) {
			...
			List<File> files = (List<File>) savedInstanceState
	...
	private void showMd5Mismatch(String md5, String calculated, final File file) {
		...
		alert.setMessage(getResources()
				.getString(R.string.md5_mismatch_summary,
		...
		alert.setNegativeButton(R.string.md5_install_anyway,
				new DialogInterface.OnClickListener() {
	...
	private void showRemoveDialog(final Item item, final File file) {
		...
		String message = context.getResources().getString(
				R.string.remove_file_summary, file.getName());
