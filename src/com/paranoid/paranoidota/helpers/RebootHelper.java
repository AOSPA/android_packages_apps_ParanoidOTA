...
public class RebootHelper {
			...
			alert.setPositiveButton(android.R.string.ok,
					new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog,
							...
							reallyShowBackupDialog(context, items, wipeData,
									wipeCaches);
						}
					});
	...
	private void reallyShowBackupDialog(final Context context,
			final String[] items, final boolean wipeData,
		...
		View view = LayoutInflater.from(context).inflate(
				...
				(ViewGroup) ((Activity) context)
						.findViewById(R.id.backup_dialog_layout));
		alert.setView(view);
		...
		final CheckBox cbSecure = (CheckBox) view
				.findViewById(R.id.androidsecure);
		...
		alert.setPositiveButton(android.R.string.ok,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int whichButton) {
						...
						reboot(context, items, wipeData, wipeCaches, text,
								backupOptions);
					}
				});
	...
	public void showRebootDialog(final Context context, final String[] items,
			final boolean backup, final boolean wipeData,
			final boolean wipeCaches) {
		...
		if (items.length == 1) {
			alert.setMessage(context.getResources().getString(
					R.string.alert_reboot_one_message));
		} else {
			alert.setMessage(context.getResources().getString(
					R.string.alert_reboot_more_message));
		}
		...
		alert.setPositiveButton(R.string.alert_reboot_now,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int whichButton) {
						...
						if (backup) {
							showBackupDialog(context, items, wipeData,
									wipeCaches);
						} else {
							reboot(context, items, wipeData, wipeCaches, null,
									null);
						}
			...
			for (int i = 0; i < recoveries.length; i++) {
				...
				try {
					...
					for (int k = 0; k < files.length; k++) {
						files[k] = mRecoveryHelper.getRecoveryFilePath(
								recoveries[i], items[k]);
					}

					String[] commands = mRecoveryHelper.getCommands(
							recoveries[i], files, items, wipeData, wipeCaches,
