...
public class DownloadHelper {
	...
	private static Runnable sUpdateProgress = new Runnable() {
		...
		public void run() {
			...
			long[] statusRom = sDownloadingRom ? getDownloadProgress(idRom,
					true) : new long[] { DownloadManager.STATUS_SUCCESSFUL, 0,
					0, -1 };
			long[] statusGapps = sDownloadingGapps ? getDownloadProgress(
					idGapps, false) : new long[] {
	...
	public static void init(Context context, DownloadCallback callback) {
		...
		if (sDownloadManager == null) {
			sDownloadManager = (DownloadManager) context
	...
	private static void checkDownloadFinished(long downloadId, boolean isRom,
		...
		long id = isRom ? sSettingsHelper.getDownloadRomId() : sSettingsHelper
				.getDownloadGappsId();
		if (id == -1L || (downloadId != 0 && downloadId != id)) {
			return;
		}
		String md5 = isRom ? sSettingsHelper.getDownloadRomMd5()
				: sSettingsHelper.getDownloadGappsMd5();
		...
		if (cursor.moveToFirst()) {
			...
			case DownloadManager.STATUS_FAILED:
				...
				sCallback.onDownloadError(sContext.getResources().getString(
						reasonText));
				break;
	...
	public static void downloadFile(final String url, final String fileName,
			final String md5, final boolean isRom) {
	...
	private static void realDownloadFile(String url, String fileName,
			String md5, boolean isRom) {
		DownloadManager.Request request = new DownloadManager.Request(
				Uri.parse(url));
		request.setNotificationVisibility(Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);
		request.setVisibleInDownloadsUi(false);
		request.setTitle(sContext.getResources().getString(
				R.string.download_title, new Object[] { fileName }));
		File file = new File(IOUtils.DOWNLOAD_PATH);
		if (!file.exists()) {
			file.mkdirs();
		}
		request.setDestinationUri(Uri.fromFile(new File(IOUtils.DOWNLOAD_PATH,
				fileName)));
	...
	private static void removeDownload(long id, boolean isRom,
			boolean removeDownload) {
		if (isRom) {
