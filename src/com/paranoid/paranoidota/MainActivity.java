...
public class MainActivity extends Activity implements UpdaterListener, DownloadCallback, OnNavigationListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mSavedInstanceState == null) {
            ...
            setState(STATE_UPDATES, true);
        } else {
            setState(mSavedInstanceState.getInt(STATE), false);
        }
    ...
    public void setState(int state, boolean animate) {
        setState(state, animate, null, null, null, false);
    }

    public void setState(int state, boolean animate, PackageInfo[] infos,
            Uri uri, String md5, boolean isRom) {
        mState = state;
        switch (state) {
            case STATE_UPDATES :
                getActionBar().setSelectedNavigationItem(0);
        ...
        switch (state) {
            case STATE_UPDATES :
                addCards(new Card[] {
                        new SystemCard(mContext, null, mRomUpdater, mGappsUpdater,
                                mRecoveryHelper.getRecoveryName()),
                        new UpdatesCard(mContext, null, mRomUpdater, mGappsUpdater) }, animate, true);
                break;
            case STATE_DOWNLOAD :
                addCards(new Card[] {
                        new DownloadCard(mContext, null, infos) }, animate, true);
                break;
            case STATE_INSTALL :
                ...
                if (!DownloadHelper.isDownloading(!isRom)) {
                    addCards(new Card[] {mInstallCard}, true, true);
                } else {
    ...
    public void addCards(Card[] cards, boolean animate, boolean remove) {
        System.out.println(animate + ", " + remove);
    ...
    public void onDownloadFinished(Uri uri, final String md5, boolean isRom) {
        ...
        if (uri == null) {
            ...
            setState(STATE_INSTALL, true, null, uri, md5, isRom);
        }
    }
}
