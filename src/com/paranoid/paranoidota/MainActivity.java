...
public class MainActivity extends Activity implements UpdaterListener, DownloadCallback, OnNavigationListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mSavedInstanceState == null) {
            ...
            mRomUpdater.setLastUpdates((PackageInfo[]) mSavedInstanceState.getSerializable(ROMS));
            mGappsUpdater.setLastUpdates((PackageInfo[]) mSavedInstanceState.getSerializable(GAPPS));
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        if (mInstallCard != null) {
            outState.putSerializable(FILES, (Serializable) mInstallCard.getFiles());
        }
    }
        ...
        switch (state) {
            ...
            case STATE_DOWNLOAD :
                if (mDownloadCard == null) {
                    ...
                    if (infos == null && mSavedInstanceState != null) {
                        infos = (PackageInfo[]) mSavedInstanceState.getSerializable(DOWNLOADING);
                        mDownloadProgress = mSavedInstanceState.getInt(DOWNLOAD_PROGRESS);
                        mDownloadCard.setInfos(infos, mDownloadProgress);
                    } else {
                        mDownloading = infos;
                    }
                }
            ...
            case STATE_INSTALL :
                if (mInstallCard == null) {
                    ...
                    if (mSavedInstanceState != null) {
                        List<File> files = (List<File>) mSavedInstanceState.getSerializable(FILES);
                        mInstallCard.setFiles(files);
                    }
                }
